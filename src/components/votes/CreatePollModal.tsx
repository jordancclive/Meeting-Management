...
import { X as CloseIcon, BarChart3Icon, UserIcon, CalendarIcon, ClockIcon, PlusIcon, MinusIcon, CheckIcon, AlertTriangleIcon } from 'lucide-react';
export const CreatePollModal = ({
  ...
  const [committee, setCommittee] = useState('Kubernetes Community');
  ...
  // Committees list
  const committees = ['Kubernetes Community', 'Security Working Group', 'Technical Steering Committee', 'Documentation Team', 'Governing Board'];
      ...
      <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center">
          <div className="flex items-center">
            <BarChart3Icon className="h-5 w-5 text-azure-600 mr-2" />
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <form onSubmit={handleSubmit}>
            ...
            {fromMeeting && <div className="mb-6 p-3 bg-azure-50 rounded-md text-sm text-azure-700 flex items-start">
                ...
                <div>
                  <p className="font-medium">
                    Creating poll from meeting: {fromMeeting.title}
                  </p>
                  <p>Attendees will be automatically added as participants.</p>
                </div>
              </div>}
            {/* Title */}
            <div className="mb-6">
              ...
              <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="E.g., Which name do you prefer for the new feature?" className={`w-full px-3 py-2 border ${errors.title ? 'border-red-500' : 'border-slate-300'} rounded-md focus:outline-none focus:ring-2 focus:ring-azure-500`} />
            ...
            <div className="mb-6">
              ...
              <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Provide additional context for participants..." rows={3} className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-azure-500" />
            </div>
            ...
            <div className="mb-6">
              ...
              {errors.options && <p className="mb-2 text-xs text-red-500">{errors.options}</p>}
              <div className="space-y-2">
                {options.map((option, index) => <div key={option.id} className="flex items-center">
                    <div className="flex-shrink-0 w-6 h-6 flex items-center justify-center mr-2 text-slate-400">
                      {index + 1}.
                    </div>
                    <input type="text" value={option.text} onChange={e => updateOptionText(option.id, e.target.value)} placeholder={`Option ${index + 1}`} className="flex-grow px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-azure-500" />
                    <button type="button" onClick={() => removeOption(option.id)} className="ml-2 p-1 text-slate-400 hover:text-slate-600 disabled:opacity-50" disabled={options.length <= 2}>
                      <MinusIcon className="h-5 w-5" />
                    </button>
                  </div>)}
              </div>
              <button type="button" onClick={addOption} className="mt-3 flex items-center text-sm text-azure-600 hover:text-azure-700">
